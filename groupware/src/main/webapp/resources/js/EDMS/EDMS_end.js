...
if (list_length > 10) {
    ...
    if (total_page != 1) {
        if (crt_page != 1) {
            ...
            if(crt_page % 10 == 1){
                EDMS_pre_btn.setAttribute("href", `/EDMS/end?emp_num=${address.slice(39, 47)}&crt_page=${parseInt(crt_page) - 1}&crt_block=${parseInt(crt_block) - 1}`)
            } else{
                EDMS_pre_btn.setAttribute("href", `/EDMS/end?emp_num=${address.slice(39, 47)}&crt_page=${parseInt(crt_page) - 1}&crt_block=${crt_block}`)
            }
        ...
        if (total_page != crt_page) {
            ...
            if (crt_page % 10 == 0) {
                EDMS_next_btn.setAttribute("href", `/EDMS/end?emp_num=${address.slice(39, 47)}&crt_page=${parseInt(crt_page) + 1}&crt_block=${parseInt(crt_block) + 1}`)
            } else {
                EDMS_next_btn.setAttribute("href", `/EDMS/end?emp_num=${address.slice(39, 47)}&crt_page=${parseInt(crt_page) + 1}&crt_block=${crt_block}`)
            }
